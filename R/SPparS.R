# Surplus production under non-equilibrium assumption for Schaefer using time series data
...
SPpar.S_opt <- function(inpars, df){
  k <- exp(inpars[1])
  B0 <- exp(inpars[2])
  r <- exp(inpars[3])
  q <- exp(inpars[4])
...
### plot fit
predicted <- SPpar.S(inpars = fitted_pars[1:4], df)
